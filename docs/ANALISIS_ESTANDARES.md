...
Estándares en Validación de Sistemas
De HCI Clásico a Seguridad en Sistemas Críticos
...
Patrón Observer (alertas de stock mínimo)	ISO/IEC 27001, ISO 9001	Facilita	El mecanismo de notificación desacoplado puede extenderse sin modificar la lógica de negocio para registrar eventos de seguridad, accesos anómalos o modificaciones críticas, alineándose con los controles de monitoreo de ISO 27001.
Patrón Strategy (movimientos: entrada, salida, ajuste)	ISO 9001, ISO/IEC 27001	Facilita	Al aislar cada tipo de movimiento en una unidad de comportamiento independiente, se simplifica la auditoría individual de operaciones y se facilitan las pruebas unitarias, en línea con los principios de trazabilidad e integridad de datos.
Borrado lógico con campo activo	ISO 9001, ISO/IEC 27001	Facilita	Garantiza que ningún dato sea eliminado físicamente de la base, preservando el historial completo del ciclo de vida de cada producto. Esto es un requisito explícito de trazabilidad en ISO 9001 y de disponibilidad de registros en ISO 27001.
Control de acceso por rol (encargado / empleado)	ISO/IEC 27001	Facilita parcialmente	La estructura de roles existe y restringe operaciones críticas. Para cumplir ISO 27001 requeriría mayor granularidad (permisos por recurso), registro de intentos de acceso fallidos y revisión periódica de privilegios.
Historial de movimientos con usuario, fecha y motivo	ISO 9001, ISO/IEC 27001	Facilita	Constituye la base de un log de auditoría funcional. Para ISO 27001 requeriría que estos registros sean inmutables (sin posibilidad de edición posterior) y protegidos contra acceso no autorizado.
Contraseñas almacenadas en texto plano	ISO/IEC 27001	Dificulta gravemente	Constituye una no conformidad crítica bajo cualquier control de seguridad de la información. Debe resolverse reemplazando el almacenamiento directo por hashing con sal (bcrypt o equivalente) antes de cualquier intento de certificación.


5. Conclusión — Certificación del Sistema
Si el equipo tuviera que certificar el sistema Stockeado bajo un estándar actual, elegiría ISO/IEC 27001 como objetivo de certificación principal. Esta elección se fundamenta en que es el estándar que implicaría los cambios más estructurales y, por lo tanto, el que mayor valor agregaría al sistema desde una perspectiva de madurez técnica y confianza operacional.
Los cambios concretos que requeriría la certificación en el diseño actual son los siguientes: reemplazar el almacenamiento de contraseñas en texto plano por hashing con sal utilizando bcrypt o el módulo werkzeug.security de Flask; externalizar la clave secreta de sesión (secret_key) a variables de entorno, eliminando su presencia en el código fuente; implementar transacciones atómicas explícitas en todas las operaciones de escritura múltiple sobre la base de datos, garantizando consistencia ante fallos concurrentes; hacer inmutables los registros del historial de movimientos, impidiendo su modificación posterior incluso por usuarios con rol de encargado; y agregar registro de eventos de acceso (inicios de sesión, intentos fallidos y operaciones críticas) en una tabla de auditoría independiente.
En términos de patrones de diseño, el Patrón Observer implementado para la generación de alertas de stock favorece directamente el cumplimiento de ISO 27001: el mismo mecanismo desacoplado puede extenderse para emitir eventos de seguridad ante accesos anómalos, modificaciones inesperadas de stock o intentos de operación fuera de los horarios habituales, sin necesidad de alterar la lógica de negocio central. El Patrón Strategy aplicado a los movimientos de stock facilita la auditoría independiente de cada tipo de operación y reduce el riesgo de que un error en una estrategia afecte el comportamiento de las demás, alineándose con el principio de integridad de los datos que la norma exige.
